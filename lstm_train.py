...
parser.add_argument('--sample-percentage',type=float,default=0.6,help='cai yang bi li')
parser.add_argument('--suffix', type=str, default='_springs5',
                    help='Suffix for training data (e.g. "_charged5"._springs5')
...
model = RecurrentBaseline(args.dims, args.hidden, args.dims,args.num_atoms, args.num_layers, args.dropout)
# model=RNNModel(args.dims,args.hidden,args.num_atoms)
# if args.load_folder:
...
def train(epoch, best_val_loss):
    ...
    for batch_idx, (data, relations) in enumerate(valid_loader):
        ...
        # static = torch.zeros_like(data)
        # static[:, :, 0::2, :] = data[:, :, 0::2, :]
        # static[:, :, 1::2, :] = static[:, :, 0:static.size(2) - 1:2, :]
...
def test():
    ...
    for batch_idx, (inputs, relations) in enumerate(test_loader):
        ...
        if args.motion or args.non_markov:
            ...
            # Baseline over multiple steps
            baseline = inputs[:, :, args.timesteps:args.timesteps + 1,:].expand_as(target)
