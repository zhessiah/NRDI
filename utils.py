...
def binary_concrete_sample(logits, tau=1, eps=1e-10):
    ...
    if logits.is_cuda:
        ...
        logistic_noise = logistic_noise.to(device)
...
def gumbel_softmax(logits, tau=1, hard=False, eps=1e-10):
    ...
    if hard:
        ...
        y_hard = y_hard.zero_().scatter_(-1, k.view(shape[:-1] + (1,)), 1.0) # k.view(shape[:-1] + (1,)) 128,20,1
...
def load_data( batch_size=1, suffix=''):
    ...
    # Reshape to: [num_sims, num_atoms, num_timesteps, num_dims]  500,5,49,2
    loc_train = np.transpose(loc_train, [0, 3, 1, 2])
    vel_train = np.transpose(vel_train, [0, 3, 1, 2])
    feat_train = np.concatenate([loc_train, vel_train], axis=3)
    edges_train = np.reshape(edges_train, [-1, num_atoms ** 2])  # [num_sims, num_atoms*num_atoms] 500,25
    ...
    # Exclude self edges
    off_diag_idx = np.ravel_multi_index(np.where(np.ones((num_atoms, num_atoms)) - np.eye(num_atoms)),
                                        [num_atoms, num_atoms])
    ...
    # return train_data_loader, valid_data_loader, loc_max, loc_min, vel_max, vel_min
...
def edge_accuracy(preds, target):
    ...
    acc = np.float(correct) / (target.size(0) * target.size(1))
...
def load_exp2_data(timesteps, num_atoms, graph, batch_size, sample_size, dims):
    ...
    for i in range(sample_size):
        if graph == 'ba':
            G = nx.barabasi_albert_graph(num_atoms, 4)
        elif graph == 'ws':
            G = nx.newman_watts_strogatz_graph(num_atoms, 4, 0.1)
        elif graph == 'rg':
            G = nx.random_regular_graph(4, num_atoms)
        A = torch.FloatTensor(nx.to_numpy_array(G))
        with torch.no_grad():
            x0 = torch.randn(num_atoms, dims).float()
            true_x = ode.odeint(GeneDynamics(A, 1), x0, t, method='dopri5')  # torch.Size([timesteps, num_atoms, 1])

        true_x = true_x.transpose(0, 1)
        true_x_all.append(true_x)
        A = A.reshape(num_atoms * num_atoms)
        A_all.append(A)

    A_all = torch.stack(A_all)
    true_x_all = torch.stack(true_x_all)

    off_diag_idx = np.ravel_multi_index(np.where(np.ones((num_atoms, num_atoms)) - np.eye(num_atoms)),[num_atoms, num_atoms])
    A_all = A_all[:, off_diag_idx]

    train_data = TensorDataset(true_x_all, A_all)
    train_data_loader = DataLoader(train_data, batch_size=batch_size)
